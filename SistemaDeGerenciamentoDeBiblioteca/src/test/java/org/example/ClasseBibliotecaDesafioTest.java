...
class ClasseBibliotecaDesafioTest {
    ...
    void testFluxoCompleto() {
        biblioteca.adicionarLivro(new ClasseLivro("111111", "Teste", "Autor", 2024));
        biblioteca.registrarUsuario(new ClasseUsuario("U003", "Carlos", "carlos@email.com"));

        assertTrue(biblioteca.emprestarLivro("111111", "U003", LocalDate.now()));
        assertFalse(biblioteca.emprestarLivro("111111", "U002", LocalDate.now()));

        double multa = biblioteca.devolverLivro("111111", LocalDate.now().plusDays(20));
        assertEquals(12.0, multa); // 6 dias de atraso * 2.0

        List<ClasseLivro> disponiveis = biblioteca.listarLivrosDisponiveis();
        assertTrue(disponiveis.stream().anyMatch(l -> l.getIsbn().equals("111111")));
    }
    ...
    void testPerformanceMultiplosEmprestimos() {
        assertTimeout(Duration.ofMillis(100), () -> {
            for (int i = 0; i < 100; i++) {
                ClasseLivro l = new ClasseLivro("L" + i, "Livro " + i, "Autor " + i, 2000 + i);
                ClasseUsuario u = new ClasseUsuario("U" + i, "Nome " + i, "email" + i + "@mail.com");
                biblioteca.adicionarLivro(l);
                biblioteca.registrarUsuario(u);
                biblioteca.emprestarLivro("L" + i, "U" + i, LocalDate.now());
            }
        });
    }
}
